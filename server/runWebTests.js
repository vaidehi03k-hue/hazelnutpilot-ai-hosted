...
export async function runWebTests({ steps, baseUrl, runId, maxRunMs = 120000 }) {
  ...
  // Important for containerized environments (Render)
  ...
  // Reasonable caps so runs never "hang"
  page.setDefaultTimeout(15000);
  page.setDefaultNavigationTimeout(20000);

  // Simple debug log written to /runs/<runId>/run.log
  const logLines = [];
  const log = (m) => {
    const line = `[${new Date().toISOString()}] ${m}`;
    logLines.push(line);
    console.log(line); // also visible in Render logs
  };
      ...
      try {
        switch (s.step) {
          case "goto":
            ...
            await page.goto(joinUrl(baseUrl, s.target || "/"), {
          ...
          case "assertText": {
            ...
            if (!asRegex(s.pattern).test(txt)) {
              throw new Error(`assertText failed: got "${txt}", expected ${s.pattern}`);
            }
            break;
          }
          ...
          case "assertUrl": {
            ...
            if (!asRegex(s.pattern).test(url)) {
              throw new Error(`assertUrl failed: "${url}" !~ ${s.pattern}`);
            }
            break;
          }
        ...
        if (status === "error") break; // stop on first failure; change to continue if you prefer
      }
    }
  } finally {
    // Save a small debug log and the video
    try { await fs.writeFile(path.join(outDir, "run.log"), logLines.join("\n")); } catch {}
